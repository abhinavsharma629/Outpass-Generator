function drafts() {
  ...
  if ($("#drafts1").html()) {
    ...
    $("#drafts1").attr("style", "display:block;");
    $("#createOutpass1").attr("style", "display:none;");
    ...
    $.ajax({
      url: "http://127.0.0.1:8000/student/savedOutpass",
      type: "GET",
      headers: {
        Authorization: "Bearer " + access_token
      },
      success: function(data) {
        var savedOutpasses = JSON.parse(data.saved_outpasses);
        console.log(savedOutpasses);
        var html = "";
        for (var i = 0; i < savedOutpasses.length; i++) {
          var from = savedOutpasses[i]["fromDate"];
          var to = savedOutpasses[i]["toDate"];
          var id = savedOutpasses[i]["outpassId"];
          var purpose = savedOutpasses[i]["purposeOfLeave"];
          ...
          html +=
            ...
            id +
            `">`;
          if (savedOutpasses[i]["approved_by_warden"] === false) {
            html += `
                          <button class="remove11" id="remove11"><i class="fa fa-trash"></i></i><b>Delete</b></button>
                          <button class="close11" id="close"><i class="fa fa-times"></i></button>`;
          }
          html += `<button class="view" id="view"><i class="fa fa-eye"></i><b>View</b></button>

                              <button class="edit" id="edit1"><i class="fa fa-pencil"></i><b>Edit</b></button>
                              <button class="send11" id="send11"><i class="fa fa-send-o"></i><b>Send</b></button>
                        </div>
        ...
        $(".view").click(function() {
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              $("#num_of_days1111").val(outpassDetails[0]["no_of_days"]);
        ...
        $(".edit").click(function() {
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              $("#purposeOfLeave1").val(outpassDetails[0]["purposeOfLeave"]);
              $("#addressWhileLeave1").val(
                outpassDetails[0]["addressWhileLeave"]
              );
              $("#num_of_days1").val(outpassDetails[0]["no_of_days"]);
              $("#fromDate1").val(outpassDetails[0]["fromDate"]);
              $("#toDate1").val(outpassDetails[0]["toDate"]);
              $("#isEmergency1").attr(
                "checked",
                outpassDetails[0]["isEmergency"]
              );
              $("#hid_inp").val(outpassDetails[0]["outpassId"]);
              $("#preview11").attr(
                "class",
                "js--image-preview js--no-default js--no-default"
              );
        ...
        $(".send11").click(function() {
          ...
          $.ajax({
            type: "PUT",
            url: "http://127.0.0.1:8000/student/sendSavedOutpass/",
            headers: {
              Authorization: "Bearer " + access_token
            },
            data: {
            ...
            success: function(data) {
              if(data.status==="200"){
                ...
                $("#load").hide();
                $("#main1111").removeClass("blur");
                alert("Successfully Sent Outpass To Your Warden!!")
              }
              else if(data.status==="404"){
                $("#load").hide();
                $("#main1111").removeClass("blur");
                alert("Error!! Your Previous Outpass is already in queue!! \nYou can't send another request until your warden reviews your outpass or you delete the outpass !!")
        ...
        function save(id) {
          var fd = new FormData();
          if (file1) {
            fd.append("whtsImg", file1);
          }
          file1 = undefined;

          fd.append("id", id);
          fd.append("id", $("#hid_inp").val());
          fd.append("reason", $("#purposeOfLeave1").val());
          fd.append("add", $("#addressWhileLeave1").val());
          fd.append("nod", $("#num_of_days1").val());
          fd.append("from", $("#fromDate1").val());
          fd.append("to", $("#toDate1").val());
          fd.append("emergency", $("#isEmergency1").prop("checked"));

          $.ajax({
            type: "PUT",
            url: "http://127.0.0.1:8000/student/editOutpass/",
            headers: {
              Authorization: "Bearer " + access_token
            },
            data: fd,
